...
describe('Network', function() {
  ...
  it('isMember() should return group id if is a member', function() {
    var network = _network();
    network.addMember('member1');
    expect(network.isMember('member1')).to.be.a('string');
  });
  it('isMember() should return null if not a member', function() {
    var network = _network();
    expect(network.isMember('member1')).to.equal(null);
  });

  it('addMember() should return null if already member', function() {
    var network = _network();
    network.addMember('member1');
    expect(network.addMember('member1')).to.equal(null);
  });
  it('addMember() should return group id on new member', function() {
    var network = _network();
    expect(network.addMember('member1')).to.be.a('string');
  });
  ...
  it('addMember() should create and add to new group if no groups', function() {
    var network = _network();
    expect(network.addMember('member1')).to.to.be.a('string');
    expect(network.getGroups().length).to.equal(1);
  });
  it('addMember() should create and add to new group if only full groups', function() {
    var network = _network({groupSize: 2});
    expect(network.addMember('member1')).to.be.a('string');
    expect(network.addMember('member2')).to.be.a('string');
    expect(network.addMember('member3')).to.be.a('string');
  ...
  it('removeMember() should remove the right member', function() {
    ...
    expect(network.isMember('member1')).to.be.a('string');
    expect(network.isMember('member2')).to.equal(null);
  });
  it('removeMember() should destroy group if emptied', function() {
    var network = _network({groupSize: 2});
    network.addMember('member1');
    network.addMember('member2');
    network.addMember('member3');
    ...
    expect(network.getGroups().length).to.equal(2);
    network.removeMember('member3');
    network.removeMember('member4');
    expect(network.getGroups().length).to.equal(1);
  });
  ...
  it('hasGroup() should return false if group doesnt exist', function() {
    var network = _network();
    expect(network.hasGroup('somegroup')).to.equal(false);
  });
  it('hasGroup() should return true if group exist', function() {
    var network = _network();
    network.addMember('member1');
    network.addMember('member2');
    var gid = network.getGroups()[0].getId();
    expect(network.hasGroup(gid)).to.equal(true);
  });

  it('removeGroup() should remove group and all its members from network and return true', function() {
    var network = _network();
    network.addMember('member1');
    network.addMember('member2');
    network.addMember('member3');
    expect(network.getGroups().length).to.equal(2);
    var gid = network.getGroups()[1].getId();
    expect(network.removeGroup(gid)).to.equal(true);
    expect(network.getGroups().length).to.equal(1);
    expect(network.isMember('member3')).to.equal(null);
  });
  it('removeGroup() should return false if group doesnt exist', function() {
    var network = _network();
    expect(network.removeGroup('somegroup')).to.equal(false);
  });
